...
def create_rls_genus_dataset(*, image_dir: Path, output_dir: Path, num_top_genera: int):
    ...
    :param image_dir: Path of an RLS image directory, containing files named `<genus>-<taxon>-<num>.<extension>`.
...
def create_rls_species_dataset(
    *, m1_csv_path: Path, image_dir: Path, output_dir: Path, num_species: int = None, min_images_per_species: int = 1
    ...
    :param image_dir: Path of an RLS image directory, containing files named `<genus>-<taxon>-<num>.<extension>`.
    :param output_dir: Path of the output directory, which must not exist.
    :param num_species: Number of species to include in the dataset. If None, all species are included.
...
def create_test_dataset(*, trip_dir: Path, output_dir: Path):
    ...
    This function attempts to filter out unlabelled files (e.g., those that include `sp`). The mapping from the original
    filenames is written to `src_to_dst.csv` in the output directory.

    :param trip_dir: Path of a trip directory, containing files named `<ID> - [species][ AND <species>]*.<jpg|JPG>`.
    :param output_dir: Path of the output directory, which must not exist.
    """
    output_dir.mkdir(parents=True)
    src_to_dst = {}
